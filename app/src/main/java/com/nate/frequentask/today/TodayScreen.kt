    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = {
                    IconButton(
                        onClick = { navController.navigate("themeList") }
                    ) {
                        Icon(
                            imageVector = Icons.AutoMirrored.Filled.List,
                            contentDescription = stringResource(
                                id = R.string.theme_list_content_description
                            )
                        )
                    }
                }
                ...
                LazyVerticalStaggeredGrid(
                    ...
                    modifier = Modifier.fillMaxSize()){
                        (themeList?.filter { theme -> theme.active && theme.tasks.any { it.isDue() } }
                            ?.sortedBy { it.created } ?: listOf<Theme>()).also { themes ->
                            items(themes) { theme ->
                                ElevatedCard(
                                    ...
                                    Column (modifier = Modifier.padding(bottom = 8.dp, start = 3.dp, end = 3.dp)){
